...
import { fetchWeatherDataAPI, WeatherDataWithLocation, LocationData } from '../api/weatherAPI';
...
export const fetchWeatherByHistory = createAsyncThunk<WeatherDataWithLocation, LocationData, { rejectValue: string }>(
  ...
  async (history, { rejectWithValue }) => {
    try {
      const data = await fetchWeatherByHistory(history);
      return data;
    } catch (error: any) {
      return rejectWithValue(error.message);
    }
...
export const saveHistory = createAsyncThunk<LocationData[], LocationData, { state: RootState }>(
  ...
  async (locationData, { getState }) => {
    ...
    const newHistory = [locationData, ...history.filter(item => locationData.name.toLowerCase() !== locationData.name.toLowerCase())].slice(0, 10);
